...
public:
        ...
        multisampleEnabled_ {false}
    {
        ...
        if(!hmd_)
        {
           ...
           // std:cout << "Using the debug hmd"<<std::endl;
        }
    ...
    ~Oculus()
    {
        std::cout<< "Oculus destructor"<<std::endl;
    ...
    void getInput()
    {
        ...
        if(sensorState_.StatusFlags & (ovrStatus_OrientationTracked	| ovrStatus_PositionTracked))
        {
            ...
            std::cout<< "Angles: "
                        ...
                        << OVR::RadToDegree(angles_[1]) << " degrees"<<std::endl;;

            std::cout << "Angles: "
                        << angles_[0] << ", "
                        << angles_[1] << ", "
                        << angles_[1] << " rad"<<std::endl;;

            std::cout << "DAngles: "
                        << OVR::RadToDegree(dAngles_[0]) << ", "
                        << OVR::RadToDegree(dAngles_[1]) << ", "
                        << OVR::RadToDegree(dAngles_[1]) << " degrees"<<std::endl;;
        }
        else
        {
            std::cout << "No input data (using debug hmd)"<<std::endl;;
        }
    }
...
protected:
    ...
    void computeSizes()
    {
        ...
        std::cout<< "Fov: " << Utils::radToDegree(2 * atan(hmdDesc_.DefaultEyeFov[0].UpTan))<<std::endl;;
